...
export default function EmpresaConfiguracionDashboardPage() {
  ...
  useEffect(() => {
    if (!isFirebaseReady || authLoading || !user?.uid) return; 
    ...
    const fetchCompany = async () => {
      ...
      if (fetchedCompany) {
        if (!(fetchedCompany.members && fetchedCompany.members[user.uid])) {
            ...
            router.push(`/dashboard/empresas`); // Ruta actualizada
            return;
        }
  ...
  if (!company) {
    return <p>Empresa no encontrada o acceso no autorizado.</p>;
  }
