...
import { TopEmisoresChart } from '@/components/facturacion/TopEmisoresChart';
...
import { getFacturasRecibidasByEmpresa, deleteFactura } from '@/lib/facturaService';
...
export default function FacturasRecibidasPage() {
  ...
  const loadFacturas = useCallback(async () => {
    ...
    try {
      setIsLoading(true);
      // Usar la función específica para obtener facturas recibidas
      const result = await getFacturasRecibidasByEmpresa(activeCompanyId);
      ...
      toast({
        title: 'Error',
        description: 'No se pudieron cargar las facturas recibidas. ' + (error instanceof Error ? error.message : ''),
        variant: 'destructive',
      });
    } finally {
      setIsLoading(false);
  ...
  // Manejar clic en un emisor del gráfico
  const handleEmisorClick = (rutEmisor: string) => {
    if (!rutEmisor) {
      // Si se envía una cadena vacía, quitar todos los filtros
      setSearchTerm('');
      setFilters(prev => ({
        ...prev,
        emisor: '',
    ...
    // Actualizar el filtro de búsqueda con el RUT del emisor
    setSearchTerm(rutEmisor);
    
    // Actualizar el filtro de emisor
    setFilters(prev => ({
      ...prev,
      emisor: rutEmisor,
      estado: 'todos' // Resetear el filtro de estado
    }));
    
    // Cambiar a la pestaña de 'todos' para mostrar todos los estados
    setActiveTab('todos');
    
    // Desplazarse al campo de búsqueda para que el usuario vea el filtro aplicado
    const searchInput = document.getElementById('search') as HTMLInputElement;
    if (searchInput) {
      searchInput.value = rutEmisor;
      searchInput.focus();
    }
  };
  ...
  useEffect(() => {
    ...
    // Filtrar por emisor (en lugar de receptor)
    if (filters.emisor) {
      const emisorTerm = filters.emisor.toLowerCase();
      result = result.filter(factura => 
        (factura.razonSocialEmisor?.toLowerCase().includes(emisorTerm) || false) ||
        (factura.rutEmisor?.toLowerCase().includes(emisorTerm) || false)
      );
    }
  ...
  return (
    <div className="container py-6 space-y-6">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight">Facturas Recibidas</h1>
          <p className="text-muted-foreground">
            Gestiona las facturas que has recibido de tus proveedores
          </p>
        </div>
        <div className="flex items-center gap-2">
          <Button onClick={() => router.push('/dashboard/facturacion/importar/recibidas')}>
        ...
        <CardContent>
          <Tabs value={activeTab} onValueChange={handleTabChange} className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4">
              <div>
                <label htmlFor="emisor" className="block text-sm font-medium text-gray-700 mb-1">Emisor</label>
                <Input
                  id="emisor"
                  placeholder="Buscar por emisor..."
                  value={filters.emisor}
                  onChange={(e) => setFilters(prev => ({ ...prev, emisor: e.target.value }))}
                />
              </div>
            ...
            <TabsContent value={activeTab} className="mt-6">
              ...
              {/* Gráfico de Top 10 Emisores */}
              {facturas.length > 0 && (
                <div className="mb-6">
                  <TopEmisoresChart 
                    facturas={facturas} 
                    onEmisorClick={handleEmisorClick}
                  />
                </div>
              )}
