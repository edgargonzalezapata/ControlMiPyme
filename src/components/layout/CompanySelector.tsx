...
export function CompanySelector() {
  ...
  if (authLoading || isLoadingCompanies || !user) {
    return (
      <div className="flex items-center gap-2 h-10 px-4 text-sm text-gray-500 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-sm animate-pulse">
  ...
  if (userCompanies.length === 0) {
    return (
      <div className="flex items-center gap-2 h-10 px-4 py-2 text-sm text-gray-600 dark:text-gray-300 bg-indigo-50 dark:bg-indigo-900/20 border border-indigo-100 dark:border-indigo-800/30 rounded-lg shadow-sm">
  ...
  return (
    <div>
      ...
      >
        <SelectTrigger className="w-auto min-w-[200px] max-w-[280px] h-10 px-4 text-base bg-white dark:bg-gray-800 border border-indigo-200 dark:border-indigo-800 hover:border-indigo-300 dark:hover:border-indigo-700 focus:ring-indigo-500 focus:ring-offset-0 focus:border-indigo-400 focus:ring-1 rounded-lg shadow-sm transition-all duration-200">
          <div className="flex items-center gap-2 truncate">
            ...
            <SelectValue>
              {isLoadingActiveCompany ? (
                ...
                <span className="truncate font-medium text-gray-800 dark:text-gray-200">{activeCompanyDetails.name}</span>
              ) : (
                <span className="text-gray-500 dark:text-gray-400">Seleccionar Empresa</span>
              )}
            </SelectValue>
          </div>
        </SelectTrigger>
