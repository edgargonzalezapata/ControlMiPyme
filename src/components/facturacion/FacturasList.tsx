...
import { FileText, CheckCircle2, Clock, XCircle, AlertCircle, Trash2, Loader2, AlertTriangle } from 'lucide-react';
...
export function FacturasList({ facturas, onFacturaClick, onEliminarFactura, showEmisor = false }: FacturasListProps) {
  ...
  const [facturaAEliminar, setFacturaAEliminar] = useState<FacturaConEstado | null>(null);
  const [isConfirmDialogOpen, setIsConfirmDialogOpen] = useState(false);
  
  const handleEliminarClick = (e: React.MouseEvent, factura: FacturaConEstado) => {
    e.stopPropagation();
  ...
  const confirmarEliminacion = async () => {
    if (!onEliminarFactura || !facturaAEliminar?.id) return;
    
    try {
      setIsDeleting(facturaAEliminar.id);
      await onEliminarFactura(facturaAEliminar);
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="pt-2">
              La factura <span className="font-semibold">{facturaAEliminar?.folio}</span> será eliminada permanentemente.
              Esta acción no se puede deshacer.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel onClick={cancelarEliminacion}>Cancelar</AlertDialogCancel>
            <AlertDialogAction 
